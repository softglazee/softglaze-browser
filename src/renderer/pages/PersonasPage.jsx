                  ...
                  {paged.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className={`group/row transition-colors ${selectedIds.has(p.id) ? 'bg-primary/5' : 'hover:bg-card/50'}`}>
                        ...
                        <td className="px-5 py-4">
                          {p.usedCount > 0
                            ? <Badge variant="green" title={(p.usedOnUrls && p.usedOnUrls.length) ? p.usedOnUrls.join(', ') : t('table.sitesUsed', { count: p.usedCount })}>♻ {t('table.reuse', { defaultValue: 'Reuse' })} · {p.usedCount}</Badge>
