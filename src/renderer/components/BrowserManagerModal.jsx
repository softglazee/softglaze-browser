...
export default function BrowserManagerModal({ onClose, onInstalled }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErr(e.message || t('browserManager.errCatalogUnreachable'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function startDownload(version) {
    ...
    catch (e) { setErr(e.message || t('browserManager.errDownloadFailed')); }
  }
  ...
  async function resumeDownload(version) {
    ...
    catch (e) { setErr(e.message || t('browserManager.errResumeFailed')); }
  }

  return (
    <div className="fixed inset-0 z-50 bg-black/60 grid place-items-center p-4" onMouseDown={onClose}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={t('browserManager.title')} tabIndex={-1} className="w-[620px] max-w-full max-h-[82vh] flex flex-col bg-popover border border-border rounded-xl shadow-2xl shadow-black/50 animate-scale-in" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-display text-[15px] font-semibold text-foreground">{t('browserManager.title')}</h2>
              <p className="text-xs text-muted-foreground">{t('browserManager.subtitle')}</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={load} className="w-8 h-8 grid place-items-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground" title={t('browserManager.refresh')}><RefreshCcw className="w-4 h-4" /></button>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          {loading ? (
            <div className="flex flex-col items-center justify-center gap-3 py-16 text-muted-foreground">
              ...
              <span className="text-sm">{t('browserManager.fetchingCatalog')}</span>
            </div>
          ) : items.length === 0 ? (
            <div className="text-center py-16 text-sm text-muted-foreground">{t('browserManager.noVersions')}</div>
          ) : items.map((it) => {
            const d = it.download;
              ...
              <div key={it.version} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-card border border-border">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {(downloading || halted) && (
                    <div className="mt-1.5 flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-muted-foreground">{halted ? `${d.state === 'paused' ? t('browserManager.paused') : t('browserManager.interrupted')} · ${d.percent || 0}%` : d.state === 'extracting' ? t('browserManager.installing') : d.state === 'queued' ? t('browserManager.queued') : `${d.percent || 0}%`}</span>
                    </div>
                  )}
                ...
                {it.installed ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shrink-0">
                    <Check className="w-3.5 h-3.5" /> {t('browserManager.installed')}
                  </span>
                ) : halted ? (
                  <button onClick={() => resumeDownload(it.version)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white bg-gradient-to-br from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 shadow-lg shadow-amber-500/25 shrink-0">
                    <Play className="w-3.5 h-3.5" /> {t('browserManager.resume')}
                  </button>
                ) : downloading ? (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-muted-foreground shrink-0">
                    <Loader2 className="w-3.5 h-3.5 animate-spin" /> {d.state === 'extracting' ? t('browserManager.installingShort') : t('browserManager.downloading')}
                  </span>
                ) : (
                  <button onClick={() => startDownload(it.version)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-lg shadow-blue-500/25 shrink-0">
                    <Download className="w-3.5 h-3.5" /> {t('browserManager.download')}
                  </button>
                )}
              </div>
            );
