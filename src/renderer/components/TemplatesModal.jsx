      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Templates" tabIndex={-1} className="w-full max-w-xl max-h-[88vh] overflow-hidden flex flex-col rounded-xl border border-border bg-popover shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-foreground font-medium text-[15px]">Profile Templates</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <h3 className="text-[13px] font-medium text-foreground mb-3 flex items-center gap-2"><Save className="w-4 h-4 text-muted-foreground" /> Save a profile as a template</h3>
          ...
          {/* Existing templates */}
          <div>
            <h3 className="text-[13px] font-medium text-foreground mb-3">Templates</h3>
            {loading ? (
              <div className="flex items-center justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-blue-500" /></div>
            ) : templates.length === 0 ? (
              <p className="text-[13px] text-muted-foreground py-4">No templates yet. Snapshot a profile above to create one.</p>
            ) : (
        ...
        <div className="flex items-center justify-end px-5 py-4 border-t border-border">
          <Button size="sm" onClick={onClose} className="bg-secondary hover:bg-elevated text-foreground border border-border">Close</Button>
        </div>
      </div>
    </div>
  );
