...
export default function DeleteProfileModal({ profile, onClose, onDeleted }) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      onDeleted();
    } catch (e) { setError(e.message || 'Failed to delete.'); setBusy(false); }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Move to Trash" tabIndex={-1} className="w-full max-w-md rounded border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-surface">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-foreground font-bold text-sm uppercase tracking-wide">Move to Trash</h2>
          </div>
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-foreground">Move <strong className="text-foreground">{profile.title}</strong> to Trash? You can restore it later unless you also wipe its data.</p>
          ...
          <div className="rounded border border-border bg-surface px-4 py-3 text-sm flex items-center gap-3">
            ...
            {loading ? (
              <span className="text-muted flex items-center gap-2"><Loader2 className="w-3.5 h-3.5 animate-spin" /> Reading stored data…</span>
            ) : info && info.exists ? (
              <span className="text-foreground">Stored browser data: <strong>{formatBytes(info.bytes)}</strong> <span className="text-muted">({info.files.toLocaleString()} files — cookies, cache, logins)</span></span>
            ) : (
              <span className="text-muted">No local browser data on disk yet.</span>
            )}
          </div>
          ...
          <label className="flex items-start gap-2.5 cursor-pointer select-none">
            ...
            <span className="text-sm text-foreground">
              Also delete cookies &amp; cache now
              <span className="block text-xs text-muted mt-0.5">Permanently wipes the data dir on disk. The profile can still be restored, but it will start logged-out and empty.</span>
            </span>
          </label>

          {wipe && (
            <div className="flex items-start gap-2 rounded border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
              <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" /> This part is irreversible — logins and cookies will be gone.
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-border bg-surface">
          ...
          <Button size="sm" variant="danger" onClick={confirm} disabled={busy}>
            ...
            {wipe ? 'Trash & wipe data' : 'Move to Trash'}
          </Button>
        </div>
      </div>
    </div>
