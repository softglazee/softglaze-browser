...
> **Status (branch `fix/top5-systemic-audit`, 2026-07-05):** the 5 systemic themes below **plus C1** are addressed. Fixed: **C1** account-registration takeover (first-run-only guard on OTP send + register); **C2** persona/autofill bridge (origin-bound, passwords no longer sent to page JS, fill plan capped); the **single-instance lock**; a **permission-enforcement pass** (delete/purge, clone/from-template, batchAddProxies, syncVendorPool, rotateProxyIp + SSRF guard, getProxyProviderCreds, systemHumanType, persona-vault handlers, export scoping, switchMember/superAdminSetup auth); **fail-open → fail-closed** (rbacPolicy unknown-kind + combined creds, licensePolicy null-expiry, secretStore seal, softglazeApi sync-throw, Gate vault-status, DbGate); and **browser-lifecycle** hardening (launch try/catch, session dedupe, close timeout + force-kill, surfaced injection failures). Also fixed: **C3** updater (https-only feed + Authenticode-gated) and **download integrity** (https + vendor-host allowlist on browser/Firefox/CRX downloads; Firefox SHA256SUMS verification; CRX size cap) — plus the related 🔵 http-scheme findings for `apiBaseUrl`/`updateFeedUrl`. Also fixed: **`xlsx` upgrade** to patched SheetJS 0.20.3 + importParser hardening (ReDoS regex, 5000-row cap, prototype-pollution guard, instruction-row data-loss). Also fixed: **authenticated SOCKS5** now works via a local auth-injecting relay (`socksRelay.js`). Also fixed: **fingerprint version-pin mismatch** — the reported Chrome major/full is now clamped to the actually-launched binary (`buildUserAgentBundle` coherence guard), so UA/Client-Hints/brand tokens can no longer disagree with the real TLS/JA4 handshake; a new `test/uaVersionCoherence.test.js` locks it in. Also fixed: **SOCKS4 misclassification** — SOCKS4 is now a first-class passthrough proxy type end-to-end (correct `socks4://` wire scheme instead of being dialed as `http://`/SOCKS5, no phantom auth), with `test/proxyScheme.test.js` covering it. Verified: 106/106 tests pass + frontend builds clean. **Not yet done** (follow-ups): the remaining low-priority 🟡/🔵 polish items.

> **Verification + remediation pass (2026-07-05, branch `fix/audit-remainder`).** Re-verified every unchecked box against the live code (many were already closed by the systemic PR but never ticked). Confirmed-already-fixed: delete/purge & clone/template & batchAddProxies & syncVendorPool & rotateProxyIp-SSRF & getProxyProviderCreds & systemHumanType permission gates; canReadRaw fail-closed + proxy combined-string redaction; secretStore.seal fail-closed; licensePolicy null-expiry; single-instance lock; launch-failure try/catch; applyToPage injectionDegraded; close-timeout+SIGKILL; double-launch dedupe; Firefox stdio/onGone; persona markUsed/fillPlan; cookieRobot scheme; Gate/DbGate/softglazeApi fail-closed. **Newly fixed this pass:** sync PUT-404-as-success + response size cap + no-Bearer-over-http + conflict no-auto-resolve; `effectivePermissions` clamp (stored perms can't escalate quota/create-flags, `test/permissionsClamp.test.js`); getProxyRotation/setProxyRotation/getProxyHealthHistory access checks; superAdminSetup pre-login claim; payment poll bound to our order + PayPal capture idempotency; remoteRelay redaction (case-insensitive + image scrub); localApi DNS-rebind Host guard; uncaughtException circuit-breaker; Firefox auth-relay socket reaping + timeout; autofillBridge readBody close/abort; proxy edit password-mask; profile SOCKS5 casing + SOCKS4; member-limit input focus; platformAccounts immutable update; Gate PayMethods + double-register guards; MacroRunModal concurrent-run. **Deferred (need migration / runtime / product decision, tracked below):** scrypt-cost + min-password-length (would lock out existing DBs/passwords), license-clock clamp + tenant-binding tightening, secretStore.open-on-failure, migration-idempotency scoping, offline `LICENSE_SECRET`/`generatePurchaseCode` (retire with the Ed25519 lease), downloader missing-Content-Length, and the low-priority renderer polish (window.prompt→modal, search debounce, GlobalPreferences guards, ProxyProviders stale-guard, cosmetic component-in-render/index-key). 116/116 tests pass.

> **Full rescan + remediation pass (2026-07-05, branch `fix/audit-remainder`).** Re-scanned the ENTIRE codebase from scratch with 7 parallel subsystem reviewers (IPC/RBAC, engine/fingerprint/proxy, firefox/persona, payments/license, crypto/db/sync, network-edge, renderer/preload), then fixed everything they surfaced plus the deferred items. Committed in 7 batches; **119/119 tests pass** (3 new dbCrypto version tests) + frontend builds clean.
>
> **CRITICAL fixed:** persona-vault theft via `__sgPersonaFillPlan` — the C2 fix was *bypassable*: the page supplied both the fill selector AND the personaId, so any site could type any persona's password into its own hidden input and read it back. Now gated by a real user gesture (`navigator.userActivation`) **and** origin-scope (a password only resolves for a persona OFFERED for the committed origin).
>
> **HIGH fixed:** `getAvailablePersonasForUrl` shipped plaintext passwords (Chromium bridge saved by `toPublicPersona`, but the Firefox loopback `/list` and the renderer channel were not) → now password-free at the source + `vault.manage`-gated + Firefox fills via a new origin-scoped `/sg-autofill/secret` endpoint (which also *un-breaks* Firefox password autofill the prior widget change silently killed); `saveProfileAsTemplate` had no access gate and copied `twoFactorSeed`/`platformAccounts` into a workspace-global template (cross-member 2FA theft); WebRTC `ProtectedRTC` wrapper self-identified via `.name`/`.toString()`; cloud-sync master key used the CONSTANT namespace as its KDF salt; the migration runner swallowed every "already exists" error yet recorded the migration applied (silent data-loss on rebuilds); the renderer had NO error boundary (any page's render error blanked the whole app).
>
> **MEDIUM fixed:** fingerprint coherence leaks (Intl.DateTimeFormat.resolvedOptions clobbering explicit zones, Date.toString non-native format, AudioBuffer.getChannelData in-place drift, mediaDevices own-vs-prototype, Worker/SharedWorker toString); versioned scrypt KDF (SGDB2, N=2^17, backward-compatible SGDB1 reads, min-password-length 4→8); `secretStore.open` fail-closed on undecryptable seal; offline-lease clock clamp; automation handlers now `assertCanAccessProfile` (macro/parallel/recorder/warmer) + `startWarmer` permission gate; `rotateProxyIp` redirect SSRF; Gate purchase-poll interval (auth-bypass after sign-out); Profiles search debounce + out-of-order guard.
>
> **LOW fixed:** license tenant-binding tightened; `clampNow` forward-jump poison capped; `pollCheckout` renderer-order fallback removed; sync conflict uses monotonic `rev` + key `..`-traversal rejected; `batchAddProxies` per-row quota; Cookie Robot http(s) scheme; Billing/`softglazeApi.recover`/AutomationPage-running renderer fixes; Chrome tree-kill on Windows; import-parse bounded (`sheetRows` + file-size); Firefox auth-relay leak on launch failure; removed dead `createProxyAuthExtension` (wrote plaintext creds to disk); `windowIcon` PowerShell single-quote escaping.
>
...
### Browser engines
- [x] Reported Chrome major (pinned ≤149) **mismatches the real launched binary** (150+) → UA/Client-Hints vs TLS/JA4 mismatch — `src/main/fingerprintGenerator.js:397` + `src/main/browserEngine.js:361` — *FIXED: `buildUserAgentBundle` now clamps the reported major AND full version to the actually-launched binary (`browser.version()`) whenever readable, so TLS/JA4/H2/JS-feature signals and the UA/Client-Hints/brand tokens always agree; the profile's pin is honored only when a matching binary is launched or the version is unreadable. Generator comments corrected (the pin is a preference/display value the engine clamps, not the wire identity). New `test/uaVersionCoherence.test.js` (7 cases) locks it in.*
...
- [x] **Authenticated SOCKS5 fails silently** (Chromium can't auth SOCKS; `page.authenticate` only answers 407) — `src/main/browserEngine.js:2230` — *FIXED: new `src/main/socksRelay.js` runs a local no-auth SOCKS5 server that injects the credentials to the upstream; Chromium points at the relay. Torn down on every session exit path.*
...
- [x] **`xlsx@0.18.5` runs on untrusted files in the main process** — CVE-2023-30533 (prototype pollution) + CVE-2024-22363 (ReDoS), fixed only in SheetJS 0.20.2+ — `src/main/importParser.js:4` — *FIXED: upgraded to SheetJS 0.20.3 (CDN tarball) + null-prototype guard on user-header keys in `parseDataRows`.*
- [x] Import ReDoS + no row cap in `parseWorkbookFile`, all main-thread → freezes the app — `src/main/importParser.js:197` — *FIXED: `parseResolution` regex de-fanged + input bounded; `parseWorkbookFile` capped at 5000 rows.*
...
- [x] `looksLikeInstructionRow` silently drops legit profiles titled "Example Corp"/"Notes…" with no error — `src/main/importParser.js:79` — *FIXED: phrase-only matching, gap-limited to pre-data rows, and skipped rows surfaced in the result.*
...
- [x] `parseProxyString` misclassifies `socks4://` as SOCKS5 — `src/main/browserEngine.js:277` — *FIXED: SOCKS4 is now a first-class passthrough type end-to-end. `parseProxyString` classifies socks4/socks4a → SOCKS4 (and strips the prefix, which the old regex missed → mangled host:port), `buildProxyServerArgument` emits `socks4://` (was `http://` for any stored SOCKS4, a total mis-dial), the proxy tester + `VALID_PROXY_TYPES`/`parseProxyType` accept SOCKS4, and SOCKS4 correctly skips the SOCKS5 auth relay + HTTP-407 path (SOCKS4 has no user/pass auth). UI type selects + badge updated. New `test/proxyScheme.test.js` (9 cases).*
